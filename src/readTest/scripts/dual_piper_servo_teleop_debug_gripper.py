...
def main():
    ...
    # 左夹爪 ID7
    # 右夹爪 ID10
    left_gripper_servo_id = int(rospy.get_param("~left_gripper_servo_id", 7))
    right_gripper_servo_id = int(rospy.get_param("~right_gripper_servo_id", 10))

    # 默认：
    # 180 deg -> closed
    # 270 deg -> open
    left_gripper_close_deg = float(rospy.get_param("~left_gripper_close_deg", 180.0))
    left_gripper_open_deg = float(rospy.get_param("~left_gripper_open_deg", 270.0))

    right_gripper_close_deg = float(rospy.get_param("~right_gripper_close_deg", 180.0))
    right_gripper_open_deg = float(rospy.get_param("~right_gripper_open_deg", 270.0))
    ...
    rospy.loginfo(
        "left gripper deg closed/open: %.1f -> %.1f",
        left_gripper_close_deg,
        left_gripper_open_deg
    )
    rospy.loginfo(
        "right gripper deg closed/open: %.1f -> %.1f",
        right_gripper_close_deg,
        right_gripper_open_deg
    )
    rospy.loginfo(
    ...
    while not rospy.is_shutdown():
        ...
        left_gripper_target = map_gripper(
            left_gripper_deg,
            left_gripper_close_deg,
            left_gripper_open_deg,
            gripper_close_m,
        ...
        right_gripper_target = map_gripper(
            right_gripper_deg,
            right_gripper_close_deg,
            right_gripper_open_deg,
            gripper_close_m,
